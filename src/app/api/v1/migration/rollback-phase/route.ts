...
export async function POST(request: NextRequest) {
  try {
    ...
    enforceMigrationRateLimit(user.id, 'rollback-phase')
    ...
    const parsed = migrationRollbackSchema.safeParse({ ...body, scope: 'phase' })

    if (!parsed.success) {
      const issue = parsed.error.issues[0]
      return NextResponse.json(
        { error: { code: 'VALIDATION_ERROR', message: issue?.message || 'Invalid phase rollback request' } },
        { status: 400 },
      )
    }
    ...
    if (!parsed.data.phase) {
      return NextResponse.json(
        { error: { code: 'VALIDATION_ERROR', message: 'phase is required for phase rollback' } },
        { status: 400 },
      )
    }
    ...
    console.error('POST /api/v1/migration/rollback-phase error:', error)
    return NextResponse.json(
      { error: { code: 'INTERNAL_ERROR', message: 'Failed to rollback migration phase' } },
      { status: 500 },
    )
  }
}
