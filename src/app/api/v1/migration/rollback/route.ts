...
export async function POST(request: NextRequest) {
  try {
    ...
    enforceMigrationRateLimit(user.id, 'rollback')
    ...
    const parsed = migrationRollbackSchema.safeParse({ ...body, scope: 'full' })

    if (!parsed.success) {
      const issue = parsed.error.issues[0]
      return NextResponse.json(
        { error: { code: 'VALIDATION_ERROR', message: issue?.message || 'Invalid rollback request' } },
        { status: 400 },
      )
    }
    ...
    console.error('POST /api/v1/migration/rollback error:', error)
    return NextResponse.json(
      { error: { code: 'INTERNAL_ERROR', message: 'Failed to rollback migration' } },
      { status: 500 },
    )
  }
}
