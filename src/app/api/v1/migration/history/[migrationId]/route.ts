...
export async function GET(
  ...
  try {
    ...
    enforceMigrationRateLimit(user.id, 'history-detail')
    ...
    console.error('GET /api/v1/migration/history/:migrationId error:', error)
    return NextResponse.json(
      { error: { code: 'INTERNAL_ERROR', message: 'Failed to fetch migration history record' } },
      { status: 500 },
    )
  }
}
